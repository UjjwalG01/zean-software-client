...
export function OutletPickerDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
        ...
        {isLoading ? (
          ...
          <div className="py-12 text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Ask an admin to create your first outlet.</p>
            </div>
            <Button onClick={() => { onOpenChange(false); navigate("/setup/outlets"); }} className="gradient-gold text-primary-foreground">
              ...
              return (
                ...
                >
                  <div className="h-32 w-full overflow-hidden bg-muted relative">
                    {img ? (
                      <img src={img} alt={o.name} className="h-full w-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    ) : (
                      <div className="h-full w-full" style={{ background: `linear-gradient(135deg, ${accent}, ${accent}88)` }} />
                    )}
                    <div className="absolute inset-0" style={{ background: `linear-gradient(180deg, transparent 40%, ${accent}cc)` }} />
                  </div>
                  <div className="p-4 space-y-2">
                    <h3 className="font-semibold font-display text-lg">{o.name}</h3>
                      ...
                      {o.serviceTypes.map((st) => {
                        ...
                        return (
                          <Badge key={st} variant="secondary" className="text-[10px]"
                            style={sd?.color ? { backgroundColor: `${sd.color}22`, color: sd.color, borderColor: `${sd.color}44` } : undefined}>
                            {sd?.name || st}
                          </Badge>
                        );
                      })}
