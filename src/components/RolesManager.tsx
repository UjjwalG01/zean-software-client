...
const RIGHT_LABELS: Record<keyof RoleRights, string> = {
  view: "VIEW",
  add: "ADD",
  change: "CHANGE",
  trash: "TRASH",
...
function emptyPermsFor(groups: PageGroup[]): RolePermissions {
  ...
  groups.forEach((g) =>
    g.pages.forEach((page) => {
      p[page.key] = { view: false, add: false, change: false, trash: false };
    }),
  );
  return p;
}
...
export function RolesManager() {
  ...
  useEffect(() => {
    getModules()
      .then((mods) => setGroups(groupsFromModules(mods)))
      .catch(() => {});
  }, []);
  ...
  const startEdit = (r: CustomRole) =>
    setEditing({ ...r, permissions: { ...emptyPerms(), ...r.permissions } });
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-semibold font-display flex items-center gap-2">
              <ShieldCheck className="h-4 w-4 text-primary" /> Custom Roles
            </h3>
            <p className="text-xs text-muted-foreground">
              Define what each role can see and do across the app. Saved to
              database.
            </p>
          </div>
        ...
        {roles.length === 0 ? (
          <div className="glass-card rounded-xl p-10 text-center text-sm text-muted-foreground">
            No custom roles yet. Click{" "}
            <span className="text-foreground font-medium">New Role</span> to
            create one.
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {roles.map((r) => {
              const total = r.isAdmin
                ? "All"
                : Object.values(r.permissions || {}).reduce(
                    (s, p) => s + RIGHTS.filter((k) => p[k]).length,
                    0,
                  );
              return (
  ...
  const allRightsForGroup = currentGroup.pages.every((p) =>
    RIGHTS.every((k) => editing.permissions[p.key]?.[k]),
  );

  const toggleAllForGroup = () => {
    const setVal = !allRightsForGroup;
    const next = { ...editing.permissions };
    currentGroup.pages.forEach((p) => {
      next[p.key] = {
        view: setVal,
        add: setVal,
        change: setVal,
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(v) =>
                setEditing({ ...editing, active: v === "yes" })
              }
            >
              <SelectTrigger>
                <SelectValue />
          ...
          <div className="space-y-2">
            ...
            <Select
              value={editing.isAdmin ? "yes" : "no"}
              onValueChange={(v) =>
                setEditing({ ...editing, isAdmin: v === "yes" })
              }
            >
              <SelectTrigger>
                <SelectValue />
              ...
              <tbody>
                <tr className="border-b border-border bg-muted/10">
                  <td className="px-4 py-2.5 font-medium flex items-center gap-2">
                    <Checkbox
                      checked={allRightsForGroup}
                      onCheckedChange={toggleAllForGroup}
                    />
                ...
                {currentGroup.pages.map((p) => {
                  const rights = editing.permissions[p.key] || {
                    view: false,
                    add: false,
                    change: false,
                    trash: false,
                    ...
                    >
                      <td className="px-4 py-2.5">
                        <label className="flex items-center gap-2 cursor-pointer">
                          <Checkbox
                            checked={allOn}
                            onCheckedChange={() => togglePageAll(p.key)}
                          />
                          <span>{p.label}</span>
                        </label>
                      </td>
                      {RIGHTS.map((r) => (
                        <td key={r} className="px-4 py-2.5 text-center">
                          <label className="inline-flex items-center gap-1.5 cursor-pointer">
                            <Checkbox
                              checked={rights[r]}
                              onCheckedChange={() => togglePageRight(p.key, r)}
                            />
