...
export function InventoryAnalytics() {
  ...
  const storeName = (id: string) => stores.find((s) => s.id === id)?.name || "Unassigned";
  ...
  const statusMix = useMemo(() => {
    const active = items.filter((i) => i.active);
    return [
      { name: "In Stock", value: active.filter((i) => stockStatus(i) === "in").length },
      { name: "Low Stock", value: active.filter((i) => stockStatus(i) === "low").length },
      { name: "Out of Stock", value: active.filter((i) => stockStatus(i) === "out").length },
    ].filter((d) => d.value > 0);
  }, [items]);
  ...
  const turnoverRows = useMemo(() => {
    const agg = new Map<string, { received: number; issued: number; adjusted: number }>();
    for (const m of movements) {
      ...
      else if (m.type === "purchase" || m.type === "opening") cur.received += Math.abs(m.quantity);
    ...
    return items
      .map((i) => {
        ...
        return {
          ...
          quantity: i.quantity,
          _value: onHandValue,
          _turnover: i.quantity > 0 ? Math.round((a.issued / i.quantity) * 100) / 100 : a.issued > 0 ? a.issued : 0,
        };
      })
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Kpi label="Total Stock Value" value={npr(kpis.totalValue)} sub={`${kpis.skus} active SKUs`} icon={<Coins className="h-5 w-5 text-primary" />} />
        <Kpi label="Units On Hand" value={kpis.totalUnits.toLocaleString()} sub={`${kpis.movements} ledger entries`} icon={<Boxes className="h-5 w-5 text-primary" />} />
        <Kpi label="At Reorder Point" value={String(kpis.low)} sub="Items at or below reorder level" icon={<AlertTriangle className="h-5 w-5 text-warning" />} />
        <Kpi label="Out of Stock" value={String(kpis.out)} sub="Needs immediate purchase" icon={<PackageX className="h-5 w-5 text-destructive" />} />
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Kpi label="Issued Value (ledger)" value={npr(kpis.issuedValue)} sub="Total consumption recorded" icon={<TrendingUp className="h-5 w-5 text-primary" />} />
        <Kpi label="Turnover Ratio" value={kpis.turnover.toFixed(2)} sub="Issued value ÷ stock value" icon={<Repeat className="h-5 w-5 text-primary" />} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="glass-card rounded-xl p-5 lg:col-span-2">
          <h3 className="font-display font-semibold mb-4">Stock Value by Category</h3>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={byCategory}>
                ...
                />
                <Bar dataKey="value" name="Valuation" fill="hsl(var(--primary))" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="glass-card rounded-xl p-5">
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={statusMix} dataKey="value" nameKey="name" innerRadius={60} outerRadius={95} paddingAngle={3}>
      ...
      <div className="glass-card rounded-xl p-5">
        <h3 className="font-display font-semibold mb-4">Stock Value by Store</h3>
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={byStore} layout="vertical">
              ...
              />
              <Bar dataKey="value" name="Valuation" fill="hsl(var(--primary))" radius={[0, 6, 6, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <PremiumReportFrame
        ...
        columns={[
          ...
          { key: "received", label: "Received", align: "right", width: "100px" },
