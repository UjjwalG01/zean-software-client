...
export default function ServiceTypesPage() {
  ...
  const reset = () => {
    setEditing(null);
    setForm({ name: "", color: "#f5b300", active: true });
  };

  const saveMutation = useMutation({
    mutationFn: async () => {
      if (editing) {
        await updateServiceType(editing.id, {
          name: form.name,
          color: form.color,
          active: form.active,
        });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Categories used to classify outlets and bookings.
          </p>
        </div>
        <Button
          onClick={() => {
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editing ? "Edit" : "Add"} Service Type</DialogTitle>
          </DialogHeader>
