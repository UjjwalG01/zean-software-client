...
const Attendance = () => {
  ...
  // Active members only
  const activeMembers = useMemo(() => members.filter((m) => m.status === "Active" || m.status === "Expiring"), [members]);
  ...
  const reportData = useMemo(() => {
    ...
    return membersToShow.map((m) => {
      const memberCheckIns = checkIns.filter(
        (c) => c.memberId === m.id && c.date >= format(reportMonth.start, "yyyy-MM-dd") && c.date <= format(reportMonth.end, "yyyy-MM-dd")
      );
      const presentDays = memberCheckIns.length;
      const totalDays = reportMonth.days.length;
      const absentDays = totalDays - presentDays;
      const rate = totalDays > 0 ? Math.round((presentDays / totalDays) * 100) : 0;
      return { member: m, presentDays, absentDays, totalDays, rate, checkInDates: new Set(memberCheckIns.map((c) => c.date)) };
    });
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <Tabs defaultValue="checkin" className="space-y-4">
        <TabsList className="bg-muted/50">
          <TabsTrigger value="checkin"><UserCheck className="h-3.5 w-3.5 mr-1" />Mark Attendance</TabsTrigger>
          <TabsTrigger value="report"><Calendar className="h-3.5 w-3.5 mr-1" />Attendance Report</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="checkin">
          ...
          <div className="glass-card rounded-xl p-4 flex flex-wrap items-center gap-3">
            ...
            <Select value={filterStatus} onValueChange={(v) => setFilterStatus(v as any)}>
              <SelectTrigger className="w-[140px] bg-muted/50 border-0"><SelectValue /></SelectTrigger>
              <SelectContent>
        ...
        <TabsContent value="report">
          ...
          <div className="glass-card rounded-xl p-4 flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <Input type="month" value={filterMonth} onChange={(e) => setFilterMonth(e.target.value)} className="w-[180px] bg-muted/50 border-0" />
            </div>
            <div className="flex items-center gap-2">
              <label className="text-sm font-medium text-muted-foreground">Member:</label>
              <Select value={filterMember} onValueChange={setFilterMember}>
                <SelectTrigger className="w-[200px] bg-muted/50 border-0"><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">All Members</SelectItem>
                  {activeMembers.map((m) => (
                  ...
                  reportData.map((r) => (
                    <TableRow key={r.member.id}>
                      ...
                      <TableCell className="text-center">
                        <Badge className="bg-success/20 text-success border-0 text-[10px]">{r.presentDays}</Badge>
                      </TableCell>
                      <TableCell className="text-center">
                        <Badge className="bg-destructive/20 text-destructive border-0 text-[10px]">{r.absentDays}</Badge>
                      </TableCell>
                      <TableCell className="text-center text-sm">{r.totalDays}</TableCell>
                      <TableCell className="text-center">
                        <Badge variant="outline" className={`text-[10px] ${r.rate >= 75 ? "text-success border-success/30" : r.rate >= 50 ? "text-warning border-warning/30" : "text-destructive border-destructive/30"}`}>
                          {r.rate}%
                        </Badge>
                      </TableCell>
                    </TableRow>
